...
def extract_info_sections(info_section_image):
    ...
    # These percentages divide the info section vertically.
    # You'll need to visually inspect your 'info_section.png' to set these accurately.
    # For example, if student ID is the top 25%, subject code is next 25%, total marks is remaining 50%.
    student_id_section_height_percentage = 0.33  # Adjust this
    subject_code_section_height_percentage = 0.33 # Adjust this
    ...
    if student_id_end_y > student_id_start_y:
        ...
        print("❌ Student ID section crop failed (dimensions invalid).")
    ...
    if subject_code_end_y > subject_code_start_y:
        ...
        print("❌ Subject Code section crop failed (dimensions invalid).")
    ...
    if total_marks_end_y > total_marks_start_y:
        ...
        print("❌ Total Marks section crop failed (dimensions invalid).")
...
if __name__ == "__main__":
    ...
    if info_section_img is not None and info_section_img.size > 0:
        ...
        # Save info sub-sections
        if student_id_img is not None:
            cv2.imwrite("extracted_student_id.png", student_id_img)
            print("💾 Saved: extracted_student_id.png")
        if subject_code_img is not None:
            cv2.imwrite("extracted_subject_code.png", subject_code_img)
            print("💾 Saved: extracted_subject_code.png")
        if total_marks_img is not None:
            cv2.imwrite("extracted_total_marks.png", total_marks_img)
            print("💾 Saved: extracted_total_marks.png")

        # Optional: Display info sub-sections (they will close with waitKey(0) at the end)
    ...
    if answer_section_img is not None and answer_section_img.size > 0:
        ...
        # Create a directory to save the answer columns if it doesn't exist
        output_dir = "answer_columns"
        os.makedirs(output_dir, exist_ok=True)

        # Save and optionally display top half columns (Questions 1-15)
        if answer_top_half_cols:
            print(f"\nSaving and Displaying Answer Top Half Columns (1-{len(answer_top_half_cols)}):")
            for i, col_img in enumerate(answer_top_half_cols):
                col_name = f"question_{i+1}.png"
                full_path = os.path.join(output_dir, col_name)
                cv2.imwrite(full_path, col_img)
                print(f"💾 Saved: {full_path}")
                cv2.imshow(f"Top Half Q{i+1}", col_img)
                cv2.waitKey(1) # Small wait to allow image to show up

        # Save and optionally display bottom half columns (Questions 16-30)
        if answer_bottom_half_cols:
            print(f"\nSaving and Displaying Answer Bottom Half Columns ({len(answer_top_half_cols)+1}-{len(answer_top_half_cols) + len(answer_bottom_half_cols)}):")
            for i, col_img in enumerate(answer_bottom_half_cols):
                col_name = f"question_{len(answer_top_half_cols) + i + 1}.png" # Continue numbering from 16
                full_path = os.path.join(output_dir, col_name)
                cv2.imwrite(full_path, col_img)
                print(f"💾 Saved: {full_path}")
                cv2.imshow(f"Bottom Half Q{len(answer_top_half_cols)+i+1}", col_img)
                cv2.waitKey(1) # Small wait to allow image to show up

        cv2.waitKey(0) # Wait for a key press to close all column windows and info windows
